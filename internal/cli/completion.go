...
var completionRoleSubcommands = []string{"draft"}
...
func buildBashCompletionScript() string {
	...
	b.WriteString("\"\n")
	b.WriteString("    local role_subcommands=\"")
	b.WriteString(strings.Join(completionRoleSubcommands, " "))
	b.WriteString("\"\n")
	...
	b.WriteString("        return\n")
	b.WriteString("    fi\n\n")
	b.WriteString("    if [ \"${words[1]}\" = \"role\" ] && [ \"$cword\" -eq 2 ]; then\n")
	b.WriteString("        COMPREPLY=( $(compgen -W \"$role_subcommands\" -- \"$cur\") )\n")
	b.WriteString("        return\n")
	b.WriteString("    fi\n\n")
...
func buildZshCompletionScript() string {
	...
	b.WriteString("    local -a top_commands new_subcommands team_subcommands goal_subcommands role_subcommands namespace_subcommands operator_subcommands gate_subcommands common_flags\n")
	...
	b.WriteString(")\n")
	b.WriteString("    role_subcommands=(")
	for i, c := range completionRoleSubcommands {
		if i > 0 {
			b.WriteString(" ")
		}
		b.WriteString(zshQuote(c))
	...
	b.WriteString("        return\n")
	b.WriteString("    fi\n\n")
	b.WriteString("    if [[ \"${words[2]}\" == \"role\" && CURRENT -eq 3 ]]; then\n")
	b.WriteString("        compadd -- \"${role_subcommands[@]}\"\n")
	b.WriteString("        return\n")
	b.WriteString("    fi\n\n")
...
func buildFishCompletionScript() string {
	...
	b.WriteString("\n")
	for _, sub := range completionRoleSubcommands {
		fmt.Fprintf(&b, "complete -c amq-squad -n \"__fish_seen_subcommand_from role\" -a %s\n", fishQuote(sub))
	}
	b.WriteString("\n")
