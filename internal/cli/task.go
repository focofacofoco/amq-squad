...
func runTaskEvent(args []string) error {
	...
	if current.Task.LifecycleGenerationRef == nil {
		return fmt.Errorf("task %s has no legacy structured lifecycle identity", current.Task.ID)
	}
	ref := *current.Task.LifecycleGenerationRef
...
// taskDoneSuccessorDispatchBinding freezes successor assignee and intent so
// the task transaction can reject intervening drift.
func taskDoneSuccessorDispatchBinding(projectDir, profile, session, sender, successorID string) (*task.SuccessorDispatchBinding, error) {
...
func runTaskTerminalTransition(projectDir, profile, session string, current task.Task, actor, reason, replacement string, event task.LifecycleEvent, now time.Time) (task.Task, error) {
	if event == task.LifecycleBlock {
		return task.BlockForProfile(projectDir, profile, session, current.ID, actor, reason, now)
	}
	return task.CancelForProfile(projectDir, profile, session, current.ID, actor, reason, replacement, now)
}
...
func runTaskTransition(args []string, verb string) error {
	...
	case "done":
		...
		if strings.TrimSpace(dispatchNext) != "" {
			successorDispatch, err = taskDoneSuccessorDispatchBinding(projectDir, profile, session, me, dispatchNext)
			if err != nil {
				break
			}
		}
	...
	case "block":
		t, err = runTaskTerminalTransition(projectDir, profile, session, current.Task, me, reason, "", task.LifecycleBlock, now)
	case "reset":
		t, err = task.ResetForProfile(projectDir, profile, session, id, me, reason, now)
	case "cancel":
		t, err = runTaskTerminalTransition(projectDir, profile, session, current.Task, me, reason, replacement, task.LifecycleCancel, now)
	case "release":
