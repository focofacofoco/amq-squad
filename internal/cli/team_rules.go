...
func renderTeamRulesWithTemplate(t team.Team, template string) (string, error) {
	...
	b.WriteString("- `amq-squad task done` changes the task status only. Send any DONE report separately as ordinary AMQ status; task completion creates no outbox, delivery state, successor dispatch, or reconciliation work.\n")
